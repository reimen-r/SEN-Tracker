...
## Commands (this Windows host: use `npm.cmd`, not `npm`/`bun` — `npm.ps1` is blocked, Bun not installed)

- `npm.cmd run dev` — dev server (tsx `server.ts` + Vite HMR) on `0.0.0.0:3000`
- `npm.cmd run lint` — gate: `tsc --noEmit && eslint .` (must be green)
- `npm.cmd run lint:tsc` / `npm.cmd run lint:eslint` — individually
- `npm.cmd run test` — Vitest once; colocate `*.test.ts` next to source
- `npm.cmd run build` — `vite build` + esbuild `server.ts` → `dist/server.cjs` (must pass)
- `npm.cmd run start` — prod server; **requires `NODE_ENV=production`** (else it boots the dev Vite middleware)
- `npm.cmd run clean` — removes `dist/` (node-based)
...
- **Data** — `src/data/venezuelaGrid.ts` (24 federal entities, incident presets, `VENEZUELA_ENTITY_IDS`), `src/services/analyzer.ts` (classification engine), `src/services/iodaApi.ts` (live fetch + v2 response parser → 0–100 normalization).
...
- **Vigilancia** auto-polls every 1/5/15 min and notifies only on **severity escalations** (banner + `Notification`). It seeds the previous-severity map from the current report on enable to avoid a false first alert.
...
| Prettier | No | — | — |
| TypeScript | Yes | `tsconfig.json` (`strict: true`, no `noUncheckedIndexedAccess`) | `npm.cmd run lint:tsc` |
